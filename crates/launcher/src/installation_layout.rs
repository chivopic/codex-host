...
impl InstalledResources {
    ...
    fn from_source_checkout(
        ...
        let node = node_override
            .map(PathBuf::from)
            .or_else(|| find_node_on_path(path_environment))
            .unwrap_or_else(|| PathBuf::from(format!("node{executable_suffix}")));
        let host_runtime = host_runtime_override.map(PathBuf::from).unwrap_or_else(|| {
            repository_root.join("packages/host-runtime/dist/main.js")
        });
        ...
        Some(Self {
            ...
            node,
            host_runtime,
            desktop_controller: repository_root.join("packages/desktop-control/dist/release-main.js"),
            renderer_extension: repository_root.join("packages/renderer-extension/dist/production.js"),
        })
    }
}
...
fn find_node_on_path(path_environment: Option<&OsStr>) -> Option<PathBuf> {
    let executable_name = format!("node{}", env::consts::EXE_SUFFIX);
    env::split_paths(path_environment?).find_map(|directory| {
...
mod tests {
    ...
    fn temporary_root(label: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        env::temp_dir().join(format!("codexhost-{label}-{}-{nonce}", process::id()))
    }
    ...
    fn resolves_resources_from_a_source_checkout() {
        let root = temporary_root("source-layout");
        let executable = root
            .join("target/debug")
            .join(format!("codexhost{}", env::consts::EXE_SUFFIX));
        let node_directory = root.join("node-bin");
        let node = node_directory.join(format!("node{}", env::consts::EXE_SUFFIX));
        write_file(&node);
        let path_environment = env::join_paths([node_directory]).expect("join PATH");

        let resources = InstalledResources::from_source_checkout(
            &executable,
            None,
            None,
            Some(path_environment.as_os_str()),
        )
        .expect("recognized source checkout");

        assert_eq!(resources.node, node);
        assert_eq!(
            resources.host_runtime,
            root.join("packages/host-runtime/dist/main.js")
        );
        assert_eq!(
            resources.shim,
        ...
        assert_eq!(
            resources.desktop_controller,
            root.join("packages/desktop-control/dist/release-main.js")
        );
        assert_eq!(
            resources.renderer_extension,
            root.join("packages/renderer-extension/dist/production.js")
        );
    ...
    fn source_checkout_prefers_runtime_overrides() {
        let root = temporary_root("source-overrides");
        let executable = root
            .join("target/release")
            .join(format!("codexhost{}", env::consts::EXE_SUFFIX));
